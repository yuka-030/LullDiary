...
test('案内文を入力後も表示し文字数を更新する', async ({ page }) => {
  ...
  await expect(count).toHaveText('0 / 100字')
  await expect(guide).toHaveText('今日あったことを書いてみてね')

  await input.fill('あいう')

  await expect(guide).toBeVisible()
  await expect(count).toHaveText('3 / 100字')

  await input.press('End')
  await input.press('Backspace')

  await expect(count).toHaveText('2 / 100字')
})

test('100字をテキスト入力としてAPIへ送信する', async ({ page }) => {
...
test('長文を切り捨てず表示し100字以内へ戻すと送信可能になる', async ({ page }) => {
  const input = page.getByRole('textbox', { name: '今日は どんな１日だった？', exact: true })
  const submit = page.getByRole('button', { name: '書けたよ', exact: true })
  ...
  await expect(page.locator('#text-input-count')).toHaveText('101 / 100字')
  await expect(page.locator('#text-input-error')).toHaveText('100字以内で入力してください')
  await expect(submit).toBeDisabled()

  await input.press('End')
  await input.press('Backspace')

  await expect(page.locator('#text-input-count')).toHaveText('100 / 100字')
...
test('日本語変換中は送信できず確定後も超過した内容を保持する', async ({ page }) => {
  ...
  await expect(input).toHaveValue('あ'.repeat(101))
  await expect(page.locator('#text-input-error')).toHaveText('100字以内で入力してください')
  await expect(submit).toBeDisabled()
})
